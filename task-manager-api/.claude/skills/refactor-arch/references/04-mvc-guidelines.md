...
## Camadas obrigatórias vs. opcionais (regra inegociável)

A refatoração para MVC **DEVE OBRIGATORIAMENTE** conter as camadas **CONTROLLER** e
**MODEL**. Uma refatoração que não separe explicitamente essas duas camadas
**não é considerada válida** e deve ser corrigida antes de finalizar a Fase 3.

- **MODEL — OBRIGATÓRIO:** toda entidade/domínio deve ter seu model isolando dados
  e persistência. Não é permitido acesso a banco/SQL fora dos models.
- **CONTROLLER — OBRIGATÓRIO:** todo fluxo de request deve passar por um controller
  que orquestra a operação. Não é permitido concentrar orquestração de negócio
  diretamente nas rotas/views.
- **VIEW / ROUTES — OPCIONAL:** recomendada para separar endpoints e
  (de)serialização HTTP, mas sua ausência **não invalida** a refatoração. Quando a
  stack já resolve o roteamento de forma idiomática, a camada de view pode ser
  simplificada ou omitida — desde que Controller e Model permaneçam separados.
...
│   └── settings.<ext>
├── models/                  # OBRIGATÓRIO — 1 arquivo por domínio; só dados + persistência
│   ├── <entidade>_model.<ext>
│   └── ...
├── views/ (ou routes/)      # OPCIONAL — endpoints + (de)serialização HTTP
│   └── routes.<ext>
├── controllers/             # OBRIGATÓRIO — orquestração do fluxo por domínio
